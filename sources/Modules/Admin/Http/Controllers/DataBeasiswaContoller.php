...
class DataBeasiswaContoller extends Controller
{
    ...
    public function showBeasiswa($params)
    {
        ...
        $cek1 = Pendaftaran::where('KodePendaftaran', $id)->where('isactive', 1)->with([
            'biodata',
            'batch',
            'jalur' => function ($q) {
                $q->with([
    ...
    public function hapusdata(Request $req)
    {
        try {
            ...
            $datapendaftaran = Pendaftaran::where('KodePendaftaran', $kode)->where('deleted_at', NULL)->selectRaw(' biodata_id ')->first();

            $databiodata = Biodata::where('biodata_id', $datapendaftaran->biodata_id)->where('isactive', '1')->selectRaw('biodata_id,akun')->first();
            // dd($kode, $datapendaftaran, $databiodata);

            $berkaspendaftaran = BerkasPendaftaran::where('kode_daftar', $kode)->where('isactive', '1')->selectRaw('id')->first();
            if ($berkaspendaftaran) {
                $hapusberkas = BerkasPendaftaran::where('id', $berkaspendaftaran->id)->update([
            ...
            $datasaudara = Saudara::where('bio_id', $datapendaftaran->biodata_id)->where('isactive', '1')->selectRaw('id')->first();
            if ($datasaudara) {
                $hapussaudara = Saudara::where('id', $datasaudara->id)->update([
                    'updated_at' => date('Y-m-d H:i:s'),
                    'updated_by' => session('session')->nip,
                    'isactive' =>  '0'
                ]);
            ...
            $datatransaksi = Transaksi::where('user_id', $databiodata->akun)->where('isactive', '1')->selectRaw('id')->first();
            if ($datatransaksi) {
                $hapustransaksi = Transaksi::where('id', $datatransaksi->id)->update([
                    'updated_at' => date('Y-m-d H:i:s'),
                    'updated_by' => session('session')->nip,
                    'isactive' =>  '0'
                ]);
            ...
            $hapusbiodata =  Biodata::where('biodata_id', $databiodata->biodata_id)->update([
                'updated_at' => date('Y-m-d H:i:s'),
                'updated_by' => session('session')->nip,
                'isactive' =>  '0'
            ]);
            $hapuspendaftaran = Pendaftaran::where('KodePendaftaran', $kode)->where('deleted_at', NULL)->update([
                'updated_at' => date('Y-m-d H:i:s'),
                'updated_by' => session('session')->nip,
                'deleted_at' => date('Y-m-d H:i:s'),
                'isactive' =>  '0'
            ]);
            $hapusakun = Master_Akun::where('akun_id', $databiodata->akun)->where('isactive', '1')->update([
                'updated_at' => date('Y-m-d H:i:s'),
                'updated_by' => session('session')->nip,
                'isactive' =>  '0'
            ]);
